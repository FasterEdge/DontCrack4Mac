...
import (
	"flag"
	"github.com/FasterEdge/DontCrack4Mac/config"
	"github.com/FasterEdge/DontCrack4Mac/core"
)

// FasterEdge - 对称、可靠、安全的多场景边缘计算框架
// https://github.com/FasterEdge
// DontCrack macOS 版
// https://github.com/FasterEdge/DontCrack4Mac
// tyza66
...
func main() {
	...
	listenAddress := flag.String("listen-address", "127.0.0.1", "HTTP监听地址（可选，默认127.0.0.1仅本机访问；对外监听必须配置密码）")    // HTTP监听地址
	...
	localLogPath := flag.String("log-path", "./logs/proc_manager/", "本地日志文件目录（可选，默认 ./logs/proc_manager/进程名）") // 本地日志文件目录，默认 ./logs/proc_manager/，实际使用时会在这个目录下创建以进程名命名的子目录来存放日志文件
