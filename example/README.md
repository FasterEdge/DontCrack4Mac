...
> macOS 默认 Apple Silicon `arm64`；若需 Intel 版改为 `GOARCH=amd64`。
...
## macOS launchd 集成
参见 `example/com.fasteredge.dontcrack.plist`（LaunchAgent）与 `example/install_launchd.sh`（安装脚本），
按需修改 `ProgramArguments` 中的 `-path` / `-args` / `-pre` 参数。