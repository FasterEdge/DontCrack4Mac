...
<h2>DontCrack4Mac</h2>
<h3>macOS Process Manager</h3>
</div>


### 1. Features

- A process manager for macOS, designed to improve the robustness, availability and timing stability of background/daemon processes
- Host a target process directly via launchd (LaunchAgent/LaunchDaemon) or the command line, avoiding service unavailability caused by application crashes
- Supports managing these process types: `binary executable` (Mach-O / universal binary), `shell script`
...
- Cross-architecture, no CGO required; supports macOS amd64 / arm64 (Apple Silicon)
- Shares the same CLI, HTTP API and file structure with the Linux (manylinux), Windows, OpenHarmony and Android versions

### 2. Basic Usage

```
./dontcrack-mac \
  -path "/opt/DontCrack4Mac/example/childproc/childproc" \
...
- Differences from the OpenHarmony / Android versions:
  - Startup banner and root path messages changed to `DontCrack_mac`
  - Detects macOS binaries via Mach-O / universal binary magic numbers
  - Uses `/bin/sh` for shell scripts and pre-commands (same as the Linux version)
  - Examples changed to launchd agents (`example/com.fasteredge.dontcrack.plist`)

### 5. launchd Integration

Put the compiled `dontcrack-mac` into `/usr/local/bin/`, then install the LaunchAgent:

```bash
sudo ./example/install_launchd.sh
launchctl list | grep dontcrack
tail -f /var/log/dontcrack/manager.stdout.log
```

See the full example at `example/com.fasteredge.dontcrack.plist`; remember to adjust the managed process path, port, pre-processing script and other arguments in `ProgramArguments` to match your environment.
