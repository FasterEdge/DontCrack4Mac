...
- macOS 平台下的进程管理器，用于提高后台/守护进程的健壮性、可用性、时序稳定性
- 通过 launchd (LaunchAgent/LaunchDaemon) 或命令行直接托管一个目标进程，避免应用自身崩溃导致服务不可用
- 支持管理这些类型的进程：`二进制可执行程序`（Mach-O / universal binary）、`shell脚本`
...
- 支持跨架构，免 CGO，支持 macOS amd64 / arm64 (Apple Silicon)
- 与 Linux (manylinux)、Windows、OpenHarmony、Android 版本共享同一套命令、HTTP 接口、文件结构
...
- macOS 默认 shell 为 `/bin/sh`，本版直接使用，与 Linux 版保持一致
- 运行的文件使用 .sh 结尾、首行包含 `#!` 都将被识别为脚本文件，由 `/bin/sh` 执行
- 二进制可执行文件通过 Mach-O / universal binary 魔数识别（支持 Apple Silicon 与 Intel）
- 开启密码时，接口请求需要在 URL 参数中携带 `password` 参数，例如 `xxx/startup?password=123456`
- 与 OpenHarmony / Android 版的差异:
  - 启动横幅、根路径消息改为 `DontCrack_mac`
  - 通过 Mach-O / universal binary 魔数识别 macOS 可执行文件
  - 使用 `/bin/sh` 执行 shell 脚本与前命令（与 Linux 版一致）
  - 示例改为 `launchd` agent (`example/com.fasteredge.dontcrack.plist`)

### 五、macOS launchd 集成

将编译好的 `dontcrack-mac` 放到 `/usr/local/bin/`，然后安装 LaunchAgent：
...
完整示例见 `example/com.fasteredge.dontcrack.plist`，记得按实际情况调整 `ProgramArguments` 中的托管进程路径、端口、预处理脚本等参数。
